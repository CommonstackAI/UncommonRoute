"""Demo: 3-way comparison UI (UncommonRoute vs ClawRouter vs Claude).

Not part of the production proxy — this is a standalone FastAPI demo that
serves `frontend/index.html` and a `/api/compare` endpoint.

Run from the repo root:

    python examples/compare_server.py

The demo binds to 127.0.0.1 by default. CORS is wide-open because the demo
is expected to run locally only; do not expose it publicly.
"""
...
@app.get("/")
def index():
    return FileResponse(str(REPO_ROOT / "frontend" / "index.html"))


if __name__ == "__main__":
    import uvicorn
    host = os.environ.get("COMPARE_HOST", "127.0.0.1")
    port = int(os.environ.get("COMPARE_PORT", "3721"))
